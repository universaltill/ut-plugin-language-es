#!/usr/bin/env bash
# Tests for scripts/check-version-bump.sh (ut-docs#1940, ported from
# ut-plugin-language-de -- ut-docs#1948).
#
...
# minimal but complete layout (manifest.json v1.0.0, locales/es.json,
...
{
  "id": "com.universaltill.language-es",
  "name": "Spanish (Español)",
  "version": "1.0.0",
  "locales": ["es"]
}
JSON
    echo '{"a.one": "Uno"}' >"${case_dir}/locales/es.json"
    echo "# Spanish pack" >"${case_dir}/README.md"
...
fresh_repo
echo '{"a.one": "Uno", "a.two": "Dos"}' >"${case_dir}/locales/es.json"
commit_change
assert_fail_containing "locale changed, no bump" \
    "locales/es.json" "manifest.json" "1.0.0" "FAIL"
...
fresh_repo
echo "# Spanish pack, updated" >"${case_dir}/README.md"
commit_change
...
echo '{"a.one": "Uno", "a.three": "Tres"}' >"${case_dir}/locales/es.json"
python3 -c "
import json
m = json.load(open('${case_dir}/manifest.json'))
m['version'] = '1.0.1'
